...
function validate(body) {
    ...
    if (!body.departureDate || !DATE_RE.test(String(body.departureDate))) return 'Invalid departure date (expected YYYY-MM-DD)';
    if (body.returnDate && !DATE_RE.test(String(body.returnDate))) return 'Invalid return date (expected YYYY-MM-DD)';
    if (body.cabin && !VALID_CABINS.has(String(body.cabin))) return 'Invalid cabin class';
    const passengers = Number(body.passengers || 1);
    if (!Number.isInteger(passengers) || passengers < 1 || passengers > 9) return 'Passengers must be 1-9';
    ...
    if (body.currency && !VALID_CURRENCIES.has(String(body.currency))) return 'Unsupported currency';
